...
const existing = foodStore.foods.find((item) => item.id === id.value)

if (!existing) {
  await router.replace('/dashboard/fnb')
}

const saving = ref(false)

const categoryOptions = computed(() => {
  const options = [...FOOD_CATEGORY_OPTIONS]
...
const form = reactive({
  name: existing?.name || '',
  category: existing?.category || FOOD_CATEGORY_OPTIONS[0]?.value || '',
...
async function handleSubmit() {
  saving.value = true
  try {
    const updated = await updateFood(Number(id.value), buildPayload())
    foodStore.upsert(mapApiFoodToFnbItem(updated))
    message.success('Đã cập nhật món')
    router.push('/dashboard/fnb')
  } catch (err) {
    message.error(err instanceof Error ? err.message : 'Không cập nhật được món')
  } finally {
    saving.value = false
  }
}
...
<template>
  <div>
    <div class="mb-4">
      ...
      <h2 class="mt-2 text-xl font-semibold text-gray-900">Cập nhật món</h2>
    </div>

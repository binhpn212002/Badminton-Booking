...
const { keyword, status, filteredData, resetFilters } = useListFilter(data, {
  getKeywordFields: (item) => [item.code, item.name],
  getStatus: (item) => item.status,
})
const { open, loading, target, askDelete, confirmDelete } = useDeleteRecord<Court>()
...
const columns: TableColumnsType<Court> = [
  ...
  { title: 'Tên sân', dataIndex: 'name', key: 'name', width: 140 },
  { title: 'Chi nhánh', dataIndex: 'branchId', key: 'branchId', width: 150 },
  { title: 'Kích thước', key: 'size', width: 120 },
  { title: 'Mặt sân', dataIndex: 'surface', key: 'surface', width: 100 },
  ...
  <div>
    <div class="mb-4 flex flex-wrap items-center gap-3">
      <a-input
        ...
        allow-clear
        placeholder="Tìm theo mã hoặc tên sân"
        class="max-w-xs"
      />
    ...
    >
      <template #bodyCell="{ column, record }">
        ...
        <template v-else-if="column.key === 'branchId'">

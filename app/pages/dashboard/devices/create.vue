...
const form = reactive({
  sku: '',
  name: '',
  category: DEVICE_CATEGORY_OPTIONS[0]?.value || '',
  stock: 0,
  price: 0,
...
async function handleSubmit() {
  saving.value = true
  try {
    const created = await createDevice(buildPayload())
    deviceStore.upsert(mapApiDeviceToDevice(created))
    message.success('Đã tạo thiết bị')
    router.push('/dashboard/devices')
  } catch (err) {
    message.error(err instanceof Error ? err.message : 'Không tạo được thiết bị')
  } finally {
    saving.value = false
  }
}
...
<template>
  <div>
    <div class="mb-4">
      ...
      <h2 class="mt-2 text-xl font-semibold text-gray-900">Thêm thiết bị</h2>
    </div>
